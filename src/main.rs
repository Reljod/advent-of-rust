use day_2::first_part;
use day_2::second_part;
use std::fs;
...
fn main() {
    ...
    let points_first_part = first_part::get_match_points(&strategy);
    println!("First Part Points: {}", points_first_part);

    let points_second_part = second_part::get_match_points(&strategy);
    println!("Second Part Points: {}", points_second_part);
}
